...
HOST_PROJECT_DIR=$(cat "${_PROJECT_ROOT}/.data/host_project_path.map" 2>/dev/null | tr -d '\n')
if [ -z "$HOST_PROJECT_DIR" ]; then
    echo "❌ host_project_path.map not found"
    echo "   Run 'docker compose up' from host first"
    exit 1
fi

DC="docker compose -f ${_PROJECT_ROOT}/docker-compose.yml --env-file $ENV_FILE -p $PROJECT"
...
if ! check_compat "$WANTED_ODOO" "$WANTED_PY" "$WANTED_PG" "$COMPAT_FILE"; then
    ...
    echo ""
    grep -E "^$WANTED_ODOO[[:space:]]*:" "$COMPAT_FILE" | while IFS=: read -r _ def_py py _ pg; do
        py=$(echo "$py" | tr -d ' ')
        pg=$(echo "$pg" | tr -d ' ')
...
if $REBUILD_DB && [ -n "$INSTALLED_PG" ] && [ "$WANTED_PG" != "$INSTALLED_PG" ]; then
    ...
    docker run --rm --user root -v ${HOST_PROJECT_DIR}/.data/volumes/db-container/pgdata:/data postgres:${INSTALLED_PG} sh -c "rm -rf /data/*"
    echo "Wiping Odoo data..."
    docker run --rm --user root -v ${HOST_PROJECT_DIR}/.data/volumes/odoo-container/odoo:/data postgres:${INSTALLED_PG} sh -c "rm -rf /data/filestore/* /data/sessions/*"
    RESTART_ODOO=true
    echo ""
fi
...
if $REBUILD_ODOO; then
    ...
    CONTAINERS_TO_UP="$CONTAINERS_TO_UP odoo"
fi
