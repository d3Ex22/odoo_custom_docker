...
| `conf VAR [value]` | | Configure .env settings |
...
| `web-terminal.conf` | Web terminal appearance (font, colors, scrollback) |
| `theme.conf` | Terminal theme and colors |
...
#### `.config/web-terminal.conf`

Web terminal (xterm.js) configuration:

```bash
WEBTERM_FONT_FAMILY="0xProto Nerd Font Mono, JetBrains Mono, monospace"
WEBTERM_FONT_SIZE=14
TMUX_CURSOR_STYLE=block      # block, underline, bar
...
```bash
# Logo colors
COLOR_PRIMARY="#2ecc71"        # Green for Utils terminal
COLOR_PRIMARY="#8be9fd"         # Cyan for Logs terminal
...
Changes to these files take effect after `rebuild` (for web-terminal/theme) or `reboot` (for odoo.conf).
...
| `conf VAR [valeur]` | | Configure les paramètres .env |
...
| `web-terminal.conf` | Apparence terminal web (police, couleurs, scrollback) |
| `theme.conf` | Thème et couleurs du terminal |
...
```bash
# Couleurs des logos
COLOR_PRIMARY="#2ecc71"        # Vert pour terminal Utils
COLOR_PRIMARY="#8be9fd"         # Cyan pour terminal Logs
...
Les modifications prennent effet après `rebuild` (pour web-terminal/theme) ou `reboot` (pour odoo.conf).
